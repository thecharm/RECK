...
class RECK(nn.Module):
    def __init__(self, max_length, pretrain_path, blank_padding=True, mask_entity=False):
        ...
        self.linear_q_tv = nn.Linear(768, self.hidden_size // 2)
        self.linear_k_tv = nn.Linear(self.hidden_size // 2, self.hidden_size // 2)
        self.linear_v_tv = nn.Linear(self.hidden_size // 2, self.hidden_size // 2)
        self.linear_q_vt = nn.Linear(self.hidden_size // 2, self.hidden_size // 2)
        self.linear_k_vt = nn.Linear(self.hidden_size // 2, self.hidden_size // 2)
        self.linear_v_vt = nn.Linear(self.hidden_size // 2, self.hidden_size // 2)
    ...
    def forward(self, token, att_mask, pos1, pos2, pic, A, W, A_rev, W_rev):
        ...
        Args:
            ...
            A: the Adjacency Matrix of the t2v knowledge graph
            W:  the Weight Matrix of the t2v knowledge graph
            A_rev: the Adjacency Matrix of the v2t knowledge graph
            W_rev: the Weight Matrix of the v2t knowledge graph
        Return:
            (B, 2H), representations for sentences
        """

        # extract knowledge features representation
        W = self.gat(W, A)
        W_rev = self.gat_rev(W_rev, A_rev)
        W = torch.sum(W, dim=1)
        W_rev = torch.sum(W_rev, dim=1)
        W = torch.cat([W, W_rev], dim=1)
        W_final = self.linear_w(W)

        # extract visual features representation
        ...
        # extract textual features representation
        ...
        pic_1 = img_feature
        pic_2 = img_feature
        hidden_k = self.linear_k_tv(hidden)
        hidden_v = self.linear_v_tv(hidden)
        pic_q = self.linear_q_tv(pic_1)
        pic_final = torch.tanh(self.att(pic_q, hidden_k, hidden_v))
        pic_final = torch.sum(pic_final, dim=1)
        pic_k = self.linear_k_vt(pic_2)
        pic_v = self.linear_v_vt(pic_2)
        hidden_q = self.linear_q_vt(hidden)
        hidden_final = torch.tanh(self.att(hidden_q, pic_k, pic_v))

        # Get entity start hidden state
        onehot_head = torch.zeros(hidden_final.size()[:2]).float().to(hidden.device)  # (B, L)
        onehot_tail = torch.zeros(hidden_final.size()[:2]).float().to(hidden.device)  # (B, L)
        onehot_head = onehot_head.scatter_(1, pos1, 1)
        onehot_tail = onehot_tail.scatter_(1, pos2, 1)
        head_hidden = (onehot_head.unsqueeze(2) * hidden_final).sum(1)  # (B, H)
        tail_hidden = (onehot_tail.unsqueeze(2) * hidden_final).sum(1)  # (B, H)
        x = torch.cat([head_hidden, tail_hidden, pic_final, W_final], -1)  # (B, 2H)
